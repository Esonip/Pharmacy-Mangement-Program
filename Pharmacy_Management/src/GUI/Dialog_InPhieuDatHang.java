...
public class Dialog_InPhieuDatHang extends JDialog {
	...
	public Dialog_InPhieuDatHang(JFrame parent, String maPDH, String maNV, String maKH, String ngayLap, String ngayGiao,
			double tongTien, String trangThaiStr, String phuongThucThanhToanStr, java.util.List<Object[]> chiTietList) {
		super(parent, "Phiếu Đặt Hàng - " + maPDH, true);
		...
		// Tiêu đề hóa đơn
		JLabel lblTieuDe = new JLabel("PHIẾU ĐẶT HÀNG");
		...
		panelThongTin.setBorder(BorderFactory.createTitledBorder("Thông Tin Phiếu Đặt Hàng"));
		panelThongTin.setFont(new Font("Segoe UI", Font.PLAIN, 14));
		contentPane.add(panelThongTin);
		panelThongTin.setLayout(new GridLayout(6, 2, 10, 10));

		JLabel lblMaHD = new JLabel("Mã Phiếu Đặt Hàng:");
		lblMaHD.setFont(new Font("Segoe UI", Font.PLAIN, 14));
		panelThongTin.add(lblMaHD);

		JLabel txtMaHD = new JLabel(maPDH);
		txtMaHD.setFont(new Font("Segoe UI", Font.PLAIN, 14));
		panelThongTin.add(txtMaHD);

		JLabel lblNgayLap = new JLabel("Ngày Đặt:");
		...
		lblNgayGiao.setFont(new Font("Segoe UI", Font.PLAIN, 14));
		panelThongTin.add(lblNgayGiao);

		JLabel txtNgayGiao = new JLabel(ngayGiao);
		txtNgayGiao.setFont(new Font("Segoe UI", Font.PLAIN, 14));
		panelThongTin.add(txtNgayGiao);
		...
		JLabel txtMaKH = new JLabel(maKH.equals("KHVL") ? "KHVL" : maKH);
		...
		panelChiTiet.setBorder(BorderFactory.createTitledBorder("Chi Tiết Phiếu Đặt Hàng"));
...
//		}
	...
	private void btnInHoaDonActionPerformed() {
        ...
        if (printerJob.printDialog()) {
            try {
                printerJob.print();
                JOptionPane.showMessageDialog(this, "In phiếu đặt hàng thành công!", "Thông Báo", JOptionPane.INFORMATION_MESSAGE);
                isPrinting = true;
                dispose();
            } catch (PrinterException ex) {
                JOptionPane.showMessageDialog(this, "Lỗi khi in phiếu đặt hàng: " + ex.getMessage(), "Lỗi In", JOptionPane.ERROR_MESSAGE);
                isPrinting = false;
                ex.printStackTrace();
            }
		} else {
