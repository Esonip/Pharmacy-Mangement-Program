...
public class Frame_HoaDon_BanThuoc extends JPanel {
	...
	public Frame_HoaDon_BanThuoc() {
		...
		lbMHoaDon.setFont(new Font("Segoe UI", Font.BOLD, 20));
		lbMHoaDon.setBounds(22, 38, 123, 40);
		...
		JDateNgayLapTim.setBounds(186, 277, 225, 40);
		...
		lblMKhchHng.setBounds(22, 194, 147, 40);
		...
		txtMaKhachHangTim.setBounds(186, 196, 225, 40);
		...
		lblNgyLpn.setBounds(22, 277, 147, 40);
		...
		lblPhuongThucThanhToan.setBounds(22, 429, 123, 40);
		...
		cbxPhuongThucTT.setBounds(186, 429, 225, 40);
		...
		btnXuatFile.setBounds(123, 21, 179, 48);
	...
	private void btnHuyAction() {
		...
		if (selectedRow != -1) {
			String maHoaDon = (String) tableModel.getValueAt(selectedRow, 0);
			int confirm = JOptionPane.showConfirmDialog(this, "Bạn có chắc chắn muốn hủy hóa đơn này?", "Hủy hóa đơn",
					JOptionPane.YES_NO_OPTION);
			if (confirm == JOptionPane.YES_OPTION) {
				if (hoaDonDao.huyHoaDon(maHoaDon)) {
					// Hoàn lại số lượng thuốc đã bán
					hoanSoLuongThuoc(maHoaDon);
	...
	private void hoanSoLuongThuoc(String maHoaDon) {
		...
		for (Object[] row : data) {
			...
			thuocGiaoDichDAO.capNhatSoLuongTon(maThuoc, -soLuong);
		}
	}
