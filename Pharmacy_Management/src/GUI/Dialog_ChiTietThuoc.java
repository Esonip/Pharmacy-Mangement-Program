...
public class Dialog_ChiTietThuoc extends JDialog {
			...
			public void keyReleased(KeyEvent e) {
				...
				if (!text.isEmpty()) {
					try {
						...
						// Thêm chữ "đ" vào cuối
						txtDonGiaBan.setText(formattedNumber + "đ");
			...
			public void keyReleased(KeyEvent e) {
				...
				if (!text.isEmpty()) {
					try {
						...
						String formattedNumber = String.format("%,d", number);
						// Thêm chữ "đ" vào cuối
						txtDonGiaNhap.setText(formattedNumber + "đ");
					} catch (NumberFormatException ex) {
    ...
    public void hienThiThuoc(String maThuoc, String tenThuoc, String donViTinh, String donGiaNhap, String donGiaBan, String hsd, String hamLuong, String soLuongTon, String soLuongThucTe ) {
    	isUpdate = true;
	...
	private void btnSaveActionPerformed() throws ParseException {
		...
		if (isUpdate) {
			thuocDAO.capNhatThuoc(maThuoc, tenThuoc, donViTinh, donGiaNhap, donGiaBan, hsdString, hamLuong, soLuongTon, soLuongThucTe);
		} else {
			thuocDAO.themThuoc(maThuoc, tenThuoc, donViTinh, donGiaNhap, donGiaBan, hsdString, hamLuong, soLuongTon, soLuongThucTe);
		}
		
		JOptionPane.showMessageDialog(Dialog_ChiTietThuoc.this, isUpdate ? "Cập nhật thông tin thuốc thành công!" : "Thêm thuốc mới thành công!");
		dispose();
	}
}
